...
from syne_tune.config_space import FiniteRange, Float, Integer, is_log_space
...
class ConfigGrammar:
    ...
    Structure:
        - Continuous values: token IDs 0 to num_numeric_tokens-1, decoded to
    ...
    The regex is built using actual token strings from the tokenizer
    vocabulary, ensuring the model only generates valid token sequences.
    """
...
class OptFormerSearcher(SingleObjectiveBaseSearcher):
    ...
    def __init__(
        ...
        if self.use_vllm:
            assert self.use_hf_checkpoint, (
                'Can only use vllm with a HF checkpoint, convert the litgpt'
                ' checkpoint first.'
            )
        ...
        if self.use_hf_checkpoint and self.use_vllm:
            self.grammar = ConfigGrammar(
            ...
            self.regex_pattern = self.grammar.build_regex()
    ...
    def suggest(self, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        if config is not None:
            ...
            if len(configs) == 0:
                logging.warning(
                    'Sampling failed, return a random configuration!'
                )
                return {k: v.sample() for k, v in self.config_space.items()}
            print(
                f'valid config: {len(configs)}/{self.n_sample_configurations}'
            )
    ...
    def _sample_n_configs(self):
        ...
        for completion in completions:
            try:
                ...
                logging.warning(
                    f'Could not sample because of error: {str(e)}, skipping'
    ...
    def _decode_config(
        ...
        if len(hp_value_tokens) != len(self.hp_cont_names) + len(
            self.hp_cat_names
        ):
        ...
        for i, (hp_name, hp_token) in enumerate(
            ...
            if is_continuous_hp:
                if self.use_hf_checkpoint:
                    int_token = int(
                        self.tokenizer.convert_ids_to_tokens(hp_token)
                    )
                else:
                ...
                tokens_per_category = {
                    ...
                    for i, cat in enumerate(
                        self.config_space[hp_name].categories
                    )
                }

                if hp_token not in tokens_per_category:
                    logging.warning(
                        f'Could not read category {hp_name}, got token'
                        f' {hp_token}.'
                    )
        ...
        for hp_name in self.hp_cat_names:
            if hp_name not in config:
                logging.warning(
                    f'Did not sample category {hp_name}, sampling randomly'
                )
                config[hp_name] = self.config_space[hp_name].sample(
                    random_state=self.random_state
                )
...
if __name__ == '__main__':
    ...
    checkpoint_dir = (
        pathlib.Path(__file__).parent.parent / 'models' / 'checkpoint'
    )
    searcher = OptFormerSearcher(
        config_space=config_space, checkpoint_dir=checkpoint_dir
    )

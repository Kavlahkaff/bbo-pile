...
def load_optformer_model(checkpoint_dir: Path, gpu_memory_utilization: float = 0.2, use_vllm: bool = True):
    """
    Load model + tokenizer once, for reuse across multiple OptFormerSearcher
...
class OptFormerSearcher(SingleObjectiveBaseSearcher):
    ...
    def __init__(
        ...
        if model is not None:
            # reuse an externally-constructed model/tokenizer (e.g. shared across seeds)
            # instead of paying vllm init/compile cost again
            self.model = model
            self.tokenizer = tokenizer
            ...
            from vllm import LLM
            from vllm.config.structured_outputs import StructuredOutputsConfig
            self.model = LLM(model=str(checkpoint_dir), structured_outputs_config=StructuredOutputsConfig(backend="xgrammar"), gpu_memory_utilization=gpu_memory_utilization)
